...
6. **Git 安全**:絕不 `--force`、絕不主動 `git push`(除非使用者明確要求)、commit 前先 `git status` 給使用者看、預設只做本地 commit。**分支策略尊重使用者既有設定**(不強制切換、可直接在使用者選定的分支含 `main` 提交)。碰到敏感檔(`.env`/`*.key`/`*secret*` 等)停下來提醒。
...
- 任何一次對 codebase 的修改/迭代完成後,依序:
  ...
  2. **主動問 git 同步**(不等使用者開口):同意 → 走 git 安全協議(`git status` 給看 → 確認分支,尊重使用者設定 → 本地 commit → 回填 CHANGELOG 的 commit hash);不同意 → 留 `⏸ 未版控`。
